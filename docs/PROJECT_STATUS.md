...
**🔑 작업 레이어 엔진 재프레이밍 (2026-07-07, 유저 확정) — Company OS로 시작.** 바이브랩스 07-06 영상("Claude Code에게 일 시키기 지쳐서 회사를 차렸습니다") + 공개 PRD 전문(wordmaster.notion.site/Company-os-395a7f332aaa80e98483c6333ae46924) 검토 결과: ① Claude Code 기반 가상 회사 패턴(헌법 md 8종 / 부서 서브에이전트 / Message·Handoff·Event 3층 / SQLite 원본 / 승인 게이트)이 우리 중립 계약 철학의 독립 검증이고, **workEvents 어휘 9종이 그 모델을 무수정 커버** (handoff→agent_message+artifact_created, 승인 게이트→decision_recorded, 상태머신→status_changed). ② **첫 엔진 = Company OS** (의존성 제로·유인 미션 적합·CrewAI 강의 선행 불필요) → **CrewAI는 건너뛸 가능성 높음, 이주는 LangGraph 직행**이 현실 경로. 첫 이주 트리거 = 무인/예약 실행 필요 (Claude Code는 세션형 엔진: durable 상태+사람 재시동이지 durable 프로세스가 아님 — 유저가 예약작업 실패로 직접 체감). ③ 슬라이스 순서: **헌법 축소판+부서 1~2개+진짜 미션 1개 → SQLite events→workEvents.push 폴링 어댑터 → videoGen(바이브랩스 콘텐츠 자동화 스킬, 단품 ₩55k 구매 확정·유저 결제 예정)을 Marketing 부서 도구로 이식.** 6부서 한꺼번에 만들지 않기(원작자 경고+sprawl 경계). 가짜 제너레이터는 M001식 미션 대본(부서 핸드오프+승인 게이트)으로 각색하면 Company OS 실물 없이도 선행 가능. 차별화 재확인: 영상의 HTML 대시보드=관리 콘솔, Orbit=감성 전시(Paperclip 검토 결론 유지). 이석현 접근 전략은 `~/.claude` 메모리 `project-vibelabs-videogen-and-leesukhyun` 참조.
**→ 07-07 슬라이스 ②a 완료:** 아바타 최소 연기 — 안무가(`src/components/isometric/acting.ts` 순수함수: 아바타별 최신 이벤트→무대지시, decision은 같은 run 직전 연기자 귀속, TTL 90s) + Player 렌더 직전 오버라이드(⚙/⏸ 말풍선·하이라이트·±1타일 결정론 서성임·⏸=부동). 클라이언트 전용(서버 write 0·Frozen 유지), 새 그림 0장, 기존 로스터만(케니5+Lyra). Playwright로 실증: Atlas 서성임 위치 변화(3초 간격 2샷), **Vega 정지+`⏸ 창업자 승인 대기: "유튜브 발행?"` 클로즈업 포착**, 비연기 아바타 회귀 확인. jest 177/vitest 18/tsc/build 그린. 관찰: 헤드리스 탭에서 반영 수 초 지연 보임(실브라우저 확인 권장, 전시 특성상 무해). 다음 = ②b(장소 이동·핸드오프 걸어가기·walkable 그래프) or B안(진짜 미니 Company OS).
**→ 07-07 저녁 — 진짜 배우 첫 입장 + 폴리시 5종:** ① **실작업 첫 emit**: Claude(이 세션)가 이석현 접촉 코멘트 초안 리서치·작성을 하며 `source:'claude-code'`로 실이벤트 push — Orion이 실작업을 연기, **진짜 승인 게이트**(코멘트 발행?)가 화면에 뜸(`docs/outreach/2026-07-07-vibelabs-comment-draft.md` A/B 2안, 창업자 결정 대기 중). 이 실험이 버그 2개를 즉시 노출: **awaiting TTL 만료**(→TTL 면제로 수정, `a334d63`) + **언프리즈 시 부동자세 글라이딩**(엔진×연기 충돌). ② **폴리시 배치 5종** (스펙/플랜 `2026-07-07-acting-polish-batch*`): 앰버 대기 글로우(Company OS 색 규칙 일치)·말풍선 40px 하강·40자·**서성임 v2(걷고-서고 리듬, 유저 "부산스럽다" 피드백 반영: 10s 주기 64% 정지, ±0.5타일)**·**엔진 우선 규칙**(엔진이 움직이면 위치·걷기 엔진 승, 연기는 말풍선·하이라이트만). jest 180 그린, Playwright 3샷 비교로 서고-걷고 리듬·앰버·40자 실증. 잔여: 극단 줌에서 말풍선 여전히 약간 붕뜸(아바타별 머리높이 메타데이터 필요, ②b감). **주의: UNFREEZE 누르면 엔진 재가동(비용) — 이날 실수로 켜져 LLM 잡담 1건 발생, `testing:stop`으로 재동결.**
**→ 07-07 슬라이스 ① 완료:** 가짜 제너레이터 가동 (`scripts/workevents/replay.mjs`, M002 대본 = Orbit 런칭 영상 미션·videoGen 예고편, `scripts/workevents/m002.json`). 티커에 "회사의 하루"가 흐르는 것 인앱 스크린샷 2장으로 확인(기본 줄→`SYS · ⏸ 창업자 승인 대기: "유튜브 발행?"` 전환 = 흐름 증명). `clearSource`로 회차 리셋(convex-test 유닛테스트), 루프/`--once`, 멱등 push(externalId `m002-r{회차}-s{step}`), 대본 계약 준수는 jest가 validator에서 어휘를 추출해 검증. 프론트 변경 0줄·Frozen 유지·테스트 전체 그린(jest 168/vitest 18). 스펙·플랜 = `docs/superpowers/{specs,plans}/2026-07-07-fake-workevents-generator*`. 다음 = ② 아바타 연기 매핑(대본의 agentName+type이 안무가 입력, Frozen-전시 안무 아키텍처 참조) or B안(진짜 최소 Company OS). 주의: 재생기 검증 시 출력을 `| head`로 파이프하면 EPIPE로 중도사망 — 파일 리다이렉트 사용.
...
**🔑 3D 리깅 레슨 + 하이브리드 파이프라인 확정 (2026-07-06 마감).** Tripo/Meshy 리깅 실전에서: ① 오토리거는 용접된 메시 전체를 "살"로 스키닝 → **매달린 액세서리(명찰·가방)는 무조건 꿀렁**, Tripo에 강체 지정 기능 없음 ② 오버사이즈 신발 → 걷기 스윙에서 뒤꿈치 관통 ③ 실측 A/B: **Tripo 스키닝 우위**(헬멧 강체 유지, Meshy는 헬멧도 꿀렁), **Meshy 메시 우위**(모델링·텍스처 분리, 더 매끄러움). → **해법 = 소스 예방** (orbit-avatar 스킬에 rig-friendly 규칙 추가: 신발 적당히·등 깨끗이·착용 액세서리 금지·핏한 옷; 소품=오버레이 결정이 실측으로 재확인됨) + **하이브리드 루트 공식 확인**: Meshy 모델링(중간폴리 GLB) → Tripo 업로드 리깅(외부 GLB 지원 공식) → 걷기 → 베이크. **매니저 리그포즈 v2 앞모습 생성·검증 완료** (`2026-07-06-office-manager-rigpose-v2.png` — 신발 축소·명찰 제거·타이 밀착, 44.5%/0.58 통과). **내일 첫 칸: 뒷모습 v2 생성 → Meshy→Tripo 하이브리드 실행 → GLB 베이크 → 게임 스케일 판정** (꿀렁이 스프라이트에서 보이면 그때만 Blender 가중치 수술 — 그록 레시피 대화에 있음, 캐논 베이스 1회). 그다음 본선: 가짜 제너레이터 → 아바타 제자리 연기 매핑.
